...
private:
  ...
  double sub_goal_x[5];
  double sub_goal_y[5];
  double sub_goal_rad[5];

  //first機体に関して
  double sub_goal_first_x[5];
  double sub_goal_first_y[5];

  //second機体に関して
  double sub_goal_second_x[5];
  double sub_goal_second_y[5];
...
void path_planning::calc_machine_position(const nav_msgs::Odometry::ConstPtr &position){
  ...
  //今回は5つのサブゴール作成
  //サブゴールの間隔を計算
  double among_sub_goal_x=diff_machine_position_x/6;
  double among_sub_goal_y=diff_machine_position_y/6;
  double among_sub_goal_rad=diff_machine_rad/6;
