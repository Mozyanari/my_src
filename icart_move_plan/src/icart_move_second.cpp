...
icart_move_second::icart_move_second(){
  //変数初期化
  //タイヤの半径[m]32.5mm
  r = 0.0325;
  //トレッド距離[m]160mm???????????????
  ...
  //購読するトピックの定義
  sub_odom= nh.subscribe("/ypspur_ros_second/odom", 5, &icart_move_second::cb_odom,this);
  sub_flag_receive=nh.subscribe("/target_point_second", 5, &icart_move_second::receive_target_point,this);
  //配布するトピックの定義
...
//関数定義-----------------------------------------------------------------------
void icart_move_second::cb_odom(const nav_msgs::Odometry::ConstPtr &msg){
  //機体の状態データ
  //機体のデータ受信
  odom = *msg;
...
//速度の計算と速度のpublish
void icart_move_second::calc_speed(void){
  //与える速度の計算
  //制御点と目標位置の差を計算
...
//目標地点の更新とflag処理
void icart_move_second::receive_target_point(const nav_msgs::Odometry::ConstPtr &data){
  //目標地点データを受信
...
{
	ros::init(argc, argv, "icart_move_second");
	icart_move_second icart_move_second;
	ros::spin();
	return 0;
}
