...
class path_planning_speed_control{
public:
  path_planning_speed_control();
private:
/*
関数定義
*/
...
現在の機体のデータ関係
*/
  /*path_planning_speed_control::cb_odom_first(const nav_msgs::Odometry::ConstPtr &msg)*/
  ...
  /*path_planning_speed_control::cb_odom_second(const nav_msgs::Odometry::ConstPtr &msg)*/
  nav_msgs::Odometry odom_second;
  //現在の機体の世界座標におけるオフセット位置
  double world_offset_position_x_second;
  double world_offset_position_y_second;

  /*path_planning_speed_control::path_planning_speed_control()*/
  //機体の最高加速度[m/s^2]
  double acc_max;
  //最高速度[m/s]
  double Max_speed;
...
path_planning_speed_control::path_planning_speed_control(){
  //購読するトピックの定義
  sub_target_point = nh.subscribe("/target_point", 5, &path_planning_speed_control::calc_machine_position,this);
  sub_odom_first = nh.subscribe("/ypspur_ros_first/odom", 5, &path_planning_speed_control::cb_odom_first,this);
  sub_arrive_position_first=nh.subscribe("/frag_data_first", 5, &path_planning_speed_control::Arrive_position_first,this);
  sub_odom_second = nh.subscribe("/ypspur_ros_second/odom", 5, &path_planning_speed_control::cb_odom_second,this);
  sub_arrive_position_second=nh.subscribe("/frag_data_second", 5, &path_planning_speed_control::Arrive_position_second,this);
...
//cb_odom_first関数定義
//機体のオドメトリデータの取得
void path_planning_speed_control::cb_odom_first(const nav_msgs::Odometry::ConstPtr &msg){
  //first機体の状態受信
  odom_first = *msg;
}
//機体の世界座標系における位置の更新
void path_planning_speed_control::update_odom_first(void){
  //first機体に関する計算----------------------------------------------------------
  //first機体の位置を代入
...
//cb_odom_second関数定義
//機体のオドメトリデータの取得
void path_planning_speed_control::cb_odom_second(const nav_msgs::Odometry::ConstPtr &msg){
  //second機体の状態受信
  odom_second = *msg;
}
//機体の世界座標系における位置の更新
void path_planning_speed_control::update_odom_second(void){
  //second機体に関する計算----------------------------------------------------------
  //second機体の位置を代入
...
void path_planning_speed_control::calc_machine_position(const nav_msgs::Odometry::ConstPtr &position){
  //目標位置を取得
  nav_msgs::Odometry control_target_position = *position;
  ...
  //目標の(x,y,θ)位置を取得
  double target_control_point_x = control_target_position.pose.pose.position.x;
  double target_control_point_y = control_target_position.pose.pose.position.y;
  double target_control_point_rad = tf::getYaw(control_target_position.pose.pose.orientation);
...
//目標地点を入力
void path_planning_speed_control::send_target_point(void){
  //first機体の次の目標位置を入力
...
//目標位置へ到達するときの機体の速度を計算
void path_planning_speed_control::send_machine_speed(void){
  //変数定義
...
void path_planning_speed_control::Arrive_position_first(const nav_msgs::Odometry::ConstPtr &position){
  //到達位置を取得
...
void path_planning_speed_control::Arrive_position_second(const nav_msgs::Odometry::ConstPtr &position){
  //到達位置を取得
...
void path_planning_speed_control::send_target_marker(void){
...
{
	ros::init(argc, argv, "path_planning_speed_control");
	path_planning_speed_control path_planning_speed_control;
	ros::spin();
	return 0;
}
