...
class multi_icart
{
  ...
  double target_position_x, target_position_y, target_rad;
...
//目標に位置に関するパラメータ
target_position_x(1.0),target_position_y(0.5),target_rad(0.0)
{
  //変数初期化
  //タイヤの半径[m]32.5mm
  r = 0.0325;
  ...
  nh.param("target_position_x",target_position_x,target_position_x);
  nh.param("target_position_y",target_position_y,target_position_y);
  nh.param("target_rad",target_rad,target_rad);
...
void multi_icart::calc_target_speed(void){
  //制御点と目標位置の差を計算
  double diff_x = (target_position_x - world_control_point_x);
  double diff_y = (target_position_y - world_control_point_y);
  double diff_rad = (target_rad - rad_multi);
